...
def medial_plot2(pts, pt_colours, tris3, colours, vbo1, vbo1_ids, vbo2, vbo2_ids, vao2, vbo3, vbo3_ids, vao3, TRIDS2):
    #drawpoints
    '''
    ...
    glColorPointer(3, GL_FLOAT, 0, len(pts.flatten()))
    
    glDrawElements(GL_POINTS, len(pts.flatten()), GL_UNSIGNED_BYTE, None)
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo3_ids)
    ...
    glDrawElements(GL_TRIANGLES, len(TRIDS2), GL_UNSIGNED_BYTE, None)
    #disable vertex arrays
    glDisableClientState(GL_VERTEX_ARRAY)
...
def main1():
    ...
    for i in test:
        verts.update(i)
    verts = np.array([[float(i) for i in j] for j in verts])
    verts2 = verts.flatten()
    
    TRIS = np.array([i for i in it.combinations(verts,3)])
    TRIS2 = TRIS.flatten()
    TRIDS = np.array([i for i in it.combinations([i for i,j in enumerate(verts)],3)])
    TRIDS2 = TRIDS.flatten()
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo1_ids)
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, len(pts2), np.array([[i] for (i,j) in enumerate(pts)]).flatten(), GL_STATIC_DRAW)
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0)
    ...
    glBufferData(GL_ARRAY_BUFFER, len(dpts2), dpts2, GL_STATIC_DRAW)
    
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo2_ids)
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, len(tris4), tris4, GL_STATIC_DRAW)
    ...
    #create vertex buffer and ids for medial axis
    vbo3 = glGenBuffers(1)
    vbo3_ids = glGenBuffers(1)
    vao3 = glGenVertexArrays(1)
    glBindVertexArray(vao3)
    
    glBindBuffer(GL_ARRAY_BUFFER, vbo3)
    glBufferData(GL_ARRAY_BUFFER, len(verts2), verts2, GL_STATIC_DRAW)
    
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo3_ids)
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, len(TRIDS2), TRIDS2, GL_STATIC_DRAW)
    ...
    while True:
        ...
        glClearDepth(1.0)
        glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT)
        medial_plot2(pts, pt_colours, tris3, colours, vbo1, vbo1_ids, vbo2, vbo2_ids, vao2, vbo3, vbo3_ids, vao3, TRIDS2)
