...
def medial_plot2(pts, pt_colours, tris3, colours, vbo1, vbo1_ids, vbo2, vbo2_ids, vao2):
    #drawpoints
    ...
    glColorPointer(3, GL_FLOAT, 0, len(pts))
    
    glDrawElements(GL_POINTS, len(pts), GL_UNSIGNED_BYTE, None)
...
def main1():
    ...
    pts, pt_cols, dpts, tris2, poles = approx_medial_axis('/home/cfillmor/topology/medial/pt_sets/mobius_500.json', 30, 0, False)
    ...
    glBindBuffer(GL_ARRAY_BUFFER, vbo1)
    glBufferData(GL_ARRAY_BUFFER, len(pts) + len(pt_colours), pts + pt_colours, GL_STATIC_DRAW)
    glBindBuffer(GL_ARRAY_BUFFER, 0)
    ...
    while True:
        ...
        glClearDepth(1.0)
        glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT)
        #medial_plot2(pts, pt_colours, tris3, colours, vbo1, vbo1_ids, vbo2, vbo2_ids, vao2)
