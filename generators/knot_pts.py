...
def mobius():
    ...
    x = b*np.cos(q*theta) * (3 + np.sin(p*theta)) - 0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
    y = b*np.sin(q*theta) * (3 + np.cos(p*theta)) - 0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
    z = a*np.sin(p*theta) - 0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
...
#'''
with open('/home/cfillmor/topology/medial/pt_sets/knot_2_3_'+str(N) + '.json', 'w') as out:
