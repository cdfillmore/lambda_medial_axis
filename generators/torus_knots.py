...
def knot(p,q):
    ...
    alpha = [np.cos(q*theta)*(3+np.cos(p*theta)), np.sin(q*theta)*(3+np.cos(p*theta)), np.sin(p*theta)]
    alphaprime = [ -q*np.sin(q*theta)*(3+np.cos(p*theta)) - p*np.sin(p*theta)*np.cos(q*theta),  q*np.cos(q*theta)*(3+np.cos(p*theta)) - p*np.sin(p*theta)*np.sin(q*theta), p*np.cos(p*theta)]
    
    temp = -alphaprime[0] * alphaprime[2] / (alphaprime[2]*alphaprime[2] + alphaprime[1]*alphaprime[1])
    
    a = [ 0, 1, -alphaprime[1] / alphaprime[2] ]
    b = [ 1, alphaprime[1]*temp/alphaprime[2], temp]
    #print(a)
    #print(b)
    ...
    a = [r*i/norma for i in a]
    b = [r*i/normb for i in b]
    
    x = alpha[0] + a[0]*np.cos(phi) + b[0]*np.sin(phi)
    y = alpha[1] + a[1]*np.cos(phi) + b[1]*np.sin(phi)
    z = alpha[2] + a[2]*np.cos(phi) + b[2]*np.sin(phi)
    
    x = x - 0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
    y = y - 0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
    z = z - 0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
    
    return [x,y,z]


N = 1000
for p,q in [(3,2), (5,2), (4,3), (5,3), (9,2), (7,4), (6,5), (9,4)]:
    for N in [100,300,500,1000,3000,5000,10000]:
        pt = np.array([knot(p,q) for i in range(N)])
        col = ['red']*N
        a = [i for i in zip(pt,col)]
        np.random.shuffle(a)
        pts,colours = (np.array([i for i,j in a]), [j for i,j in a])
        #'''
        with open('/home/cfillmor/topology/medial/pt_sets/knot_'+ str(p) + '_' + str(q) + '_' + str(N) + '.json', 'w') as out:
