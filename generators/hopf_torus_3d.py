...
def elip0():
    ...
    deg1 = np.random.randint(0,361)
    deg2 = np.random.randint(0,361)
    theta = deg1*np.pi/180
    phi = deg2*np.pi/180
    
    x = (c + a * np.cos(theta)) * np.cos(phi) - 0.5*rand_const + rand_const*np.random.rand(1,1)[0][0] + 8
    y = (c + a * np.cos(theta)) * np.sin(phi) - 0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
...
#'''
with open('/home/cfillmor/topology/medial/pt_sets/hopf_torus_'+str(N) + '.json', 'w') as out:
