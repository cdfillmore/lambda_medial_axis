...
def knot():
    ...
    a = [ 0, 1, -(4*t**4 - 26*t) / ( (7/10)*t**6 - (25/2)*t**4 + (192*3/10)*t*t - 432/10) ]
    b = [ 1, (2*t**4 - (17*6/5)*t*t + 28) * (4*t**3 -26*t) / ((4*t**3 - 26*t)**2 + ((7/10)*t**6 - (25/2)*t**4 + (192*3/10)*t*t - 432/10)**2), (-2*t**4 + (17*6/5)*t*t - 28) * ((7/10)*t**6 - (25/2)*t**4 + (192*3/10)*t*2 - 432/10) / ((4*t**3 - 26*t)**2 + ((7/10)*t**6 - (25/2)*t**4 + (192*3/10)*t*t - 432/10)**2) ]
    #print(a)
    #print(b)
    ...
    x = (2/5)*(t**5 - 17*t**3 + 70*t)
    y = t**4 - 13*t*t
    z = (1/10)*(t**7 - 25*t**5 + 192*t**3 - 432*t)
    
    x += r*(a[0]*np.cos(theta) + b[0]*np.sin(theta))
    y += r*(a[1]*np.cos(theta) + b[1]*np.sin(theta))
    z += r*(a[2]*np.cos(theta) + b[2]*np.sin(theta))
    
    x += -0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
    y += -0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
    z += -0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
...
#'''
with open('/home/cfillmor/topology/medial/pt_sets/knot_polynomial_'+str(N) + '.json', 'w') as out:
