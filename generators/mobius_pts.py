...
def mobius():
    ...
    theta = deg*np.pi/180
    width = -b/2 + b*np.random.rand()
    
    x = (a + width * np.cos(0.5*theta)) * np.cos(theta) - 0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
    y = (a + width * np.cos(0.5*theta)) * np.sin(theta) - 0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
    z = width*np.sin(0.5*theta) - 0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
...
for N in [100,300,500,1000,3000,5000,10000]:
    pt = np.array([mobius() for i in range(N)])
    col = ['red']*N
    a = [i for i in zip(pt,col)]
    np.random.shuffle(a)
    pts,colours = (np.array([i for i,j in a]), [j for i,j in a])
    
    with open('/home/cfillmor/topology/medial/pt_sets/mobius_'+str(N) + '.json', 'w') as out:
