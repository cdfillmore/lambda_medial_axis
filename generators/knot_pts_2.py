...
def knot():
    ...
    a = [ 0, 1, (-np.sin(theta) + 4*np.sin(2*theta))/(3*np.cos(3*theta)) ]
    b = [ 1, -(np.cos(theta) + 4*np.cos(2*theta))*(-np.sin(theta) + 4*np.sin(2*theta))/((-np.sin(theta) + 4*np.sin(2*theta))**2 + 9*np.cos(3*theta)*np.cos(3*theta)), (np.cos(theta) + 4*np.cos(2*theta)) * 3*np.cos(3*theta)/((-np.sin(theta) + 4*np.sin(2*theta))**2 + 9*np.cos(3*theta)*np.cos(3*theta))]
    #print(a)
    #print(b)
    ...
    x = np.sin(theta) + 2*np.sin(2*theta) + a[0]*np.cos(phi) + b[0]*np.sin(phi)
    y = np.cos(theta) - 2*np.cos(2*theta) + a[1]*np.cos(phi) + b[1]*np.sin(phi)
    z = -np.sin(3*theta) + a[2]*np.cos(phi) + b[2]*np.sin(phi)
...
for N in [100,300,500,1000,3000,5000,10000]:
    ...
    #'''
    with open('/home/cfillmor/topology/medial/pt_sets/knot_torus_'+str(N) + '.json', 'w') as out:
