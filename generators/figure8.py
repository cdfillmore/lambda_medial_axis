...
def knot():
    ...
    alpha = [np.cos(q*theta)*(2+np.cos(p*theta)), np.sin(q*theta)*(2+np.cos(p*theta)), np.sin(2*p*theta)]
    alphaprime = [ -q*np.sin(q*theta)*(2+np.cos(p*theta)) - p*np.sin(p*theta)*np.cos(q*theta),  q*np.cos(q*theta)*(2+np.cos(p*theta)) - p*np.sin(p*theta)*np.sin(q*theta), 2*p*np.cos(2*p*theta)]
...
for N in [100,300,500,1000,3000,5000,10000]:
    pt = np.array([knot() for i in range(N)])
    col = ['red']*N
    a = [i for i in zip(pt,col)]
    np.random.shuffle(a)
    pts,colours = (np.array([i for i,j in a]), [j for i,j in a])
    #'''
    with open('/home/cfillmor/topology/medial/pt_sets/knot_figure8_' + str(N) + '.json', 'w') as out:
