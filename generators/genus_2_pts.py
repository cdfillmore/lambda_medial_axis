...
def torus():
    ...
    x = (b + a * np.cos(theta)) * np.cos(phi) - 0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
    y = (b + a * np.cos(theta)) * np.sin(phi) - 0.5*rand_const + rand_const*np.random.rand(1,1)[0][0]
    z = a*np.sin(theta)
...
for N in [100,300,500,1000,3000,5000,10000]:
    ...
    pt2 = [[i[0]-12 , i[1], i[2]] for i in pt if i[0]<12]
    pt = np.array([torus() for i in range(N)])
    pt3 = [[i[0]+12 , i[1], i[2]] for i in pt if i[0]>-12]
    pt = np.array(pt2 + pt3)
    N2 = len(pt)
    col = ['red']*N2
    ...
    #'''
    with open('/home/cfillmor/topology/medial/pt_sets/genus_2_'+str(N) + '.json', 'w') as out:
